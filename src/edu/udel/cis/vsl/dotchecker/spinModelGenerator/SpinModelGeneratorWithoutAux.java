...
public class SpinModelGeneratorWithoutAux {
	private PrintStream out;

	public SpinModelGeneratorWithoutAux(PrintStream out) {
		this.out = out;
	}
	...
	public void generateSpinModelForState(DotAutomatonState state,
			int automatonId, Set<String> visibleActions) {
		...
		if (numTransitions == 1) {
			...
			generateSpinModelForTransition(transition,
					...
					visibleActions.contains(transition.getLabel()), "");
		} else {
			out.print("\n\t\t" + "if");
			for (DotAutomatonTransition transition : transitions) {
				out.print("\n\t\t" + "::");
	...
	public void generateSpinModelForTransition(
			...
			int automatonId, boolean visible, String prefix) {
		String label = transition.getLabel();

		if (synch && !label.equals(""))
			generateSpinModelForSynchTransition(transition, stateId,
					automatonId, visible, prefix);
		else
			generateSpinModelForRegularTransition(transition, stateId, visible,
					"");

	}

	public void generateSpinModelForRegularTransition(
			DotAutomatonTransition transition, int stateId, boolean visible,
			String prefix) {
		...
		out.print("atomic {");
		if (visible)
			out.print(" lastAction = " + (label.equals("") ? "nop" : label)
					+ ";");
